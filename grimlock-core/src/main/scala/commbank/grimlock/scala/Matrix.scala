...
import commbank.grimlock.framework.environment.tuner.{ Default, Reducers, Tuner }
...
import commbank.grimlock.scala.distance.PairwiseDistance
import commbank.grimlock.scala.distribution.ApproximateDistribution
import commbank.grimlock.scala.environment.Context
import commbank.grimlock.scala.environment.tuner.ScalaImplicits._
import commbank.grimlock.scala.statistics.Statistics
...
/** Case class for matrix operations using a `List[Cell[P]]`. */
case class Matrix[
  P <: HList
](
  ...
  def get[
    ...
    .tunedJoin(tuner, positions.map { case p => (p, ()) })
    .map { case (_, (c, _)) => c }
  ...
  def measure[
    ...
    List(Cell(Position(Nat.toInt[D].toLong), Content(DiscreteSchema[Long](), dist.size.toLong)))
  }
  ...
  def mutate[
    ...
    .tunedLeftJoin(tuner, positions.map { case p => (p, ()) })
    .flatMap {
...
/** Rich wrapper around a `List[Cell[V1 :: HNil]]`. */
case class Matrix1D[
  V1 <: Value[_]
](
  ...
  def saveAsIV[
    ...
    data
      .map { case c => (c.position, c) }
      .tunedJoin(jt, saveDictionary(_0, context, file, dictionary, separator, jt))
...
/** Rich wrapper around a `List[Cell[V1 :: V2 :: HNil]]`. */
case class Matrix2D[
  V1 <: Value[_],
  V2 <: Value[_]
](
  ...
  def saveAsCSV[
    ...
    if (writeHeader)
      columns
        ...
        .tunedSaveAsText(context, Default(), header.format(file))
  ...
  def saveAsIV[
    ...
    data
      ...
      .tunedJoin(jt, saveDictionary(_1, context, file, dictionary, separator, jt))
  ...
  private def saveVW[
    ...
    dict
      ...
      .tunedSaveAsText(context, Default(), dictionary.format(file))

    val features = pivoted
      .flatMapWithValue(dict.head) { case ((key, lst), dct) =>
        dct.map { case d =>
          (
            key,
            lst
    ...
    val weighted = importance match {
      case Some(imp) => features
        .tunedJoin(jt, imp.map { case c => (c.position, c) })
    ...
    val examples = labels match {
      case Some(lab) => weighted
        .tunedJoin(jt, lab.map { case c => (c.position, c) })
...
/** Rich wrapper around a `List[Cell[V1 :: V2 :: V3 :: HNil]]`. */
case class Matrix3D[
  V1 <: Value[_],
  V2 <: Value[_],
  V3 <: Value[_]
  ...
  def saveAsIV[
    ...
    data
      ...
      .tunedJoin(jt, saveDictionary(_2, context, file, dictionary, separator, jt))
...
/** Rich wrapper around a `List[Cell[V1 :: V2 :: V3 :: V4 :: HNil]]`. */
case class Matrix4D[
  V1 <: Value[_],
  V2 <: Value[_],
  V3 <: Value[_],
  ...
  def saveAsIV[
    ...
    data
      ...
      .tunedJoin(jt, saveDictionary(_3, context, file, dictionary, separator, jt))
...
/** Rich wrapper around a `List[Cell[V1 :: V2 :: V3 :: V4 :: V5 :: HNil]]`. */
case class Matrix5D[
  V1 <: Value[_],
  V2 <: Value[_],
  V3 <: Value[_],
  ...
  def saveAsIV[
    ...
    data
      ...
      .tunedJoin(jt, saveDictionary(_4, context, file, dictionary, separator, jt))
...
/** Rich wrapper around a `List[Cell[V1 :: V2 :: V3 :: V4 :: V5 :: V6 :: HNil]]`. */
case class Matrix6D[
  V1 <: Value[_],
  V2 <: Value[_],
  V3 <: Value[_],
  ...
  def saveAsIV[
    ...
    data
      ...
      .tunedJoin(jt, saveDictionary(_5, context, file, dictionary, separator, jt))
...
/** Rich wrapper around a `List[Cell[V1 :: V2 :: V3 :: V4 :: V5 :: V6 :: V7 :: HNil]]`. */
case class Matrix7D[
  V1 <: Value[_],
  V2 <: Value[_],
  V3 <: Value[_],
  ...
  def saveAsIV[
    ...
    data
      ...
      .tunedJoin(jt, saveDictionary(_6, context, file, dictionary, separator, jt))
...
/** Rich wrapper around a `List[Cell[V1 :: V2 :: V3 :: V4 :: V5 :: V6 :: V7 :: V8 :: HNil]]`. */
case class Matrix8D[
  V1 <: Value[_],
  V2 <: Value[_],
  V3 <: Value[_],
  ...
  def saveAsIV[
    ...
    data
      ...
      .tunedJoin(jt, saveDictionary(_7, context, file, dictionary, separator, jt))
...
/** Rich wrapper around a `List[Cell[V1 :: V2 :: V3 :: V4 :: V5 :: V6 :: V7 :: V8 :: V9 :: HNil]]`. */
case class Matrix9D[
  V1 <: Value[_],
  V2 <: Value[_],
  V3 <: Value[_],
  ...
  def saveAsIV[
    ...
    data
      ...
      .tunedJoin(jt, saveDictionary(_8, context, file, dictionary, separator, jt))
...
trait MatrixXD[P <: HList] extends Persist[Cell[P]] {
  ...
  def fillHeterogeneous[
    ...
    naturalDomain(dt)
      ...
      .tunedJoin(vt, values.map { case c => (c.position, c.content) })
  ...
  protected def saveDictionary[
    ...
    val numbered = coordinates(dim, tuner)
      .zipWithIndex
      .map { case (c, i) => (Position(c :: HNil), i.toLong) }

    numbered
      .map { case (Position(c :: HNil), i) => c.toShortString + separator + i }
      .tunedSaveAsText(context, Default(), dictionary.format(file, Nat.toInt[D]))

    numbered
  }
}

/** Case class for methods that change the number of dimensions using a `List[Cell[P]]`. */
case class MultiDimensionMatrix[
  ...
  def expand[
    ...
    val keys: List[(Position[ev2.Q], Value[_])] = data
      .collect { case c if (c.position(dim) equ coordinate) => (c.position.remove(dim), c.content.value) }

    data
      .collect { case c if (c.position(dim) neq coordinate) => (c.position.remove(dim), c) }
      .tunedLeftJoin(tuner, keys)
...
private object Util {
  def pivot[
    ...
    val columns = data
      .map { case c => slice.remainder(c.position) }
      .groupBy(_ => ())
      .map { case (_, l) => l.distinct.sorted(Position.ordering[R]()) }
      .toList

    val pivoted = data
      .map { case c => (slice.selected(c.position), Map(slice.remainder(c.position) -> c)) }
      .tunedReduce(tuner, _ ++ _)
      .flatMapWithValue(columns.head) { case ((key, map), opt) =>
