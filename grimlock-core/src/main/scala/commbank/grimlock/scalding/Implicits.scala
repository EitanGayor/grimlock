...
case object EnvironmentImplicits extends FwEnvironmentImplicits[Context]  {
  ...
  implicit def eToU[X : Context.D](value: Context.E[X])(implicit ctx: Context): Context.U[X] = value.toTypedPipe
}
...
case object MatrixImplicits extends FwMatrixImplicits[Context] {
  ...
  implicit def tuple1ToMatrix[
    ...
    IterablePipe(list.map { case (t1, c) => Cell(Position(t1), c) })
  )
  ...
  ): Matrix1D[Value[T1]] = Matrix1D(IterablePipe(list.map { case (t1, c) => Cell(Position(t1), c) }))
  ...
  implicit def tuple2ToMatrix[
    ...
    IterablePipe(list.map { case (t1, t2, c) => Cell(Position(t1, t2), c) })
  )
  ...
  implicit def tuple3ToMatrix[
    ...
    IterablePipe(list.map { case (t1, t2, t3, c) => Cell(Position(t1, t2, t3), c) })
  )
  ...
  implicit def tuple4ToMatrix[
    ...
    IterablePipe(list.map { case (t1, t2, t3, t4, c) => Cell(Position(t1, t2, t3, t4), c) })
  )
  ...
  implicit def tuple5ToMatrix[
    ...
    IterablePipe(list.map { case (t1, t2, t3, t4, t5, c) => Cell(Position(t1, t2, t3, t4, t5), c) })
  )
...
case object PositionImplicits extends FwPositionImplicits[Context] {
  implicit def tToU[
    ...
    ctx: Context
  ): Context.U[Position[Coordinates1[T]]] = IterablePipe(List(Position(t)))
  ...
  implicit def listTToU[
    ...
    ctx: Context
  ): Context.U[Position[Coordinates1[T]]] = IterablePipe(l.map { case t => Position(t) })
